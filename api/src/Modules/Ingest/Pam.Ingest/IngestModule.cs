...
using Pam.Ingest.Transactions.Partitioning;
...
public static class IngestModule
{
    ...
    )
    {
        ...
        // IPublishEndpoint.
        services
            .AddOptions<IngestReconciliationOptions>()
            .Bind(configuration.GetSection(IngestReconciliationOptions.SectionName));
        services.AddScoped<IOutboxReconciler, IngestOutboxReconciler>();

        // Daily partition lifecycle: extend future weekly boundaries +
        // refresh hot-partition stats. Replaces the SQL Agent job.
